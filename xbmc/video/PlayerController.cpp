...
void CPlayerController::OnSliderChange(void *data, CGUISliderControl *slider)
{
      ...
      m_sliderAction == ACTION_SUBTITLE_VSHIFT_UP || m_sliderAction == ACTION_SUBTITLE_VSHIFT_DOWN)
  {
    ...
    slider->SetTextValue(CGUIDialogSettings::FormatDecibel(slider->GetFloatValue(), 1.0f));
  else
    slider->SetTextValue(CGUIDialogSettings::FormatDelay(slider->GetFloatValue(), 0.025f));
