...
CFileItem::CFileItem(const CEpgInfoTag& tag) : URIUtilsComp<CFileItem>(*this)
{
  Initialize();
...
CFileItem::CFileItem(const CPVRRecording& record) : URIUtilsComp<CFileItem>(*this)
{
  Initialize();
...
bool CFileItem::IsFileFolder(EFileFolderType types) const
{
  ...
  if(types & always_type)
  {
    ...
    || IsRAR()
    || IsRSS()
    || IsType(".ogg|.oga|.nsf|.sid|.sap|.xsp|.mka")
    || IsAudioBook()
