...
*/
  ...
  std::string xbmcBinPath, xbmcPath;
  ...
  else
  {
    ...
    std::string strTempPath = xbmcPath;
...
bool CApplication::InitDirectoriesOSX()
{
  ...
  if (m_bPlatformDirectories)
  {
    ...
    CSpecialProtocol::SetXBMCPath(xbmcPath);
    #if defined(TARGET_DARWIN_IOS)
      CSpecialProtocol::SetHomePath(userHome + "/" + std::string(DarwinGetXbmcRootFolder()) + "/XBMC");
      CSpecialProtocol::SetMasterProfilePath(userHome + "/" + std::string(DarwinGetXbmcRootFolder()) + "/XBMC/userdata");
    #else
    ...
    #if defined(TARGET_DARWIN_IOS)
      std::string strTempPath = URIUtils::AddFileToFolder(userHome,  std::string(DarwinGetXbmcRootFolder()) + "/XBMC/temp");
    #else
      std::string strTempPath = URIUtils::AddFileToFolder(userHome, ".xbmc/");
    ...
    #if defined(TARGET_DARWIN_IOS)
      strTempPath = userHome + "/" + std::string(DarwinGetXbmcRootFolder());
    #else
  ...
  else
  {
    ...
    std::string strTempPath = URIUtils::AddFileToFolder(xbmcPath, "portable_data/temp");
    CSpecialProtocol::SetTempPath(strTempPath);
...
void CApplication::StartVideoScan(const std::string &strDirectory, bool scanAll)
{
  if (m_videoInfoScanner->IsScanning())
    return;
...
void CApplication::StartMusicScan(const std::string &strDirectory, int flags)
{
  if (m_musicInfoScanner->IsScanning())
    return;
