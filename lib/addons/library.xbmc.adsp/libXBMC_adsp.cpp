...
DLLEXPORT void* ADSP_register_me(void *hdl)
{
  CB_ADSPLib *cb = NULL;
  if (!hdl)
    fprintf(stderr, "libXBMC_adsp-ERROR: ADSPLib_register_me is called with NULL handle !!!\n");
  else
  {
    cb = ((AddonCB*)hdl)->ADSPLib_RegisterMe(((AddonCB*)hdl)->addonData);
    if (!cb)
      fprintf(stderr, "libXBMC_adsp-ERROR: ADSPLib_register_me can't get callback table from XBMC !!!\n");
  }
  return cb;
}
