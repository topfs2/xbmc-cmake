...
{
  CStdString strSQL = PrepareSQL("select bookmark from audiobook where file='%s'",
                                 item.GetPath().c_str());
...
bool CMusicDatabase::GetResumeBookmarkForAudioBook(const CStdString& path, int& bookmark)
{
  CStdString strSQL = PrepareSQL("SELECT bookmark FROM audiobook WHERE file='%s'",
                                 path.c_str());
  if (!m_pDS->query(strSQL.c_str()) || m_pDS->num_rows() == 0)
...
{
  CStdString strSQL = PrepareSQL("SELECT strBook, strAuthor, file, duration, bookmark FROM audiobook");
  if (!m_pDS->query(strSQL.c_str()) || m_pDS->num_rows() == 0)
    return false;
...
int CMusicDatabase::GetAudiobookCount(const Filter &filter)
{
  try
  {
    ...
    CStdString strSQL = "select count(idBook) as NumSongs from audiobook";
    if (!CDatabase::BuildSQL(strSQL, filter, strSQL))
      return false;
